...
HTML = r'''<title>Dierengeluiden</title>
...
<style>
  ...
  body{
    min-height:100%;
    background:linear-gradient(180deg,var(--sky) 0%,#B6E3F7 40%,var(--grass) 100%);
    color:var(--ink);
  ...
  .wrap{
    position:relative;z-index:2;box-sizing:border-box;padding:14px;
    padding-block:calc(env(safe-area-inset-top,0px) + 12px) calc(env(safe-area-inset-bottom,0px) + 14px);
    max-width:1000px;margin:0 auto;
  }
  h1{text-align:center;font-weight:800;font-size:clamp(1.3rem,5vw,2rem);
    margin:0 0 10px;color:#fff;text-shadow:0 3px 0 rgba(61,44,78,.25);text-wrap:balance;}
  .subtitle{text-align:center;margin:2px 0 14px;font-weight:600;
    font-size:clamp(1rem,3.8vw,1.25rem);color:#fff;opacity:.98;
    text-shadow:0 2px 0 rgba(61,44,78,.2);}
  ...
  .tab{
    ...
    font-size:clamp(.95rem,3.4vw,1.1rem);color:var(--ink);
    background:rgba(255,255,255,.55);border-radius:999px;padding:9px 18px;
    box-shadow:0 4px 0 rgba(61,44,78,.14);transition:transform .08s ease;
  }
  ...
  .animal{
    ...
    background:var(--card);border-radius:26px;padding:14px 6px 12px;
    display:flex;flex-direction:column;align-items:center;gap:2px;
    box-shadow:0 8px 0 rgba(61,44,78,.16),0 12px 22px rgba(61,44,78,.14);
  ...
  .emoji{font-size:clamp(3rem,14vw,4.4rem);line-height:1;
    filter:drop-shadow(0 4px 3px rgba(61,44,78,.18));}
  ...
  .replay{
    appearance:none;border:none;cursor:pointer;font-family:inherit;display:none;
  ...
  /* scene */
  #sceneWrap{width:100%;}
  #sceneWrap svg{width:100%;height:auto;display:block;border-radius:22px;
    box-shadow:0 10px 26px rgba(61,44,78,.22);}
  ...
  .flash{position:fixed;z-index:6;pointer-events:none;transform:translate(-50%,-50%);
    font-weight:800;font-size:clamp(1.8rem,9vw,3.4rem);color:#fff;
    text-shadow:0 3px 0 var(--pink),0 6px 12px rgba(61,44,78,.35);
...
<main class="wrap">
  ...
  <div class="tabs small" id="viewTabs">
    ...
    <button id="tabScene" class="tab" type="button">🌳 Boerderij</button>
  </div>

  <p class="subtitle" id="subtitle">Tik op een dier!</p>
  ...
  <section id="board">
    ...
    <div id="sceneWrap" hidden>__SCENE__</div>
  </section>

  <p class="stars" id="stars" aria-live="polite"></p>
  <p class="credit">Echte dieropnames · ESC-50 (CC BY-NC) · tekening: illustratie</p>
</main>
...
(function(){
  "use strict";
  var animals = [
    {emoji:"🐄", name:"Koe",    key:"koe"},
    {emoji:"🐶", name:"Hond",   key:"hond"},
    {emoji:"🐱", name:"Poes",   key:"poes"},
    {emoji:"🐷", name:"Varken", key:"varken"},
    {emoji:"🐑", name:"Schaap", key:"schaap"},
    {emoji:"🐔", name:"Kip",    key:"kip"},
    {emoji:"🐓", name:"Haan",   key:"haan"},
    {emoji:"🐸", name:"Kikker", key:"kikker"},
    {emoji:"🐦‍⬛", name:"Kraai", key:"kraai"},
    {emoji:"🐝", name:"Bij",    key:"bij"}
  ];
  var colors=["#FFE5EC","#E5F6FF","#FFF3D6","#E8FBE0","#F3E8FF","#FFEAD6","#E0F7F4","#FDE7F3","#EAF0FF","#FFF0E8"];
  var byKey={}; animals.forEach(function(a){byKey[a.key]=a;});
  var idx={}; animals.forEach(function(a,i){idx[a.key]=i;});
  ...
  function initAudio(){
    ...
    try{
      ...
      Object.keys(SOUNDS).forEach(function(k){
        var raw=SOUNDS[k],b64=raw.indexOf(",")>=0?raw.split(",")[1]:raw;
        try{ actx.decodeAudioData(b64buf(b64),function(buf){buffers[k]=buf;},function(){}); }catch(e){}
      });
    }catch(e){ actx=null; }
  }
  function resume(){ if(actx&&actx.state==="suspended") actx.resume(); }
  function playSample(key,cb){
    if(!actx||!buffers[key]){ if(cb)cb(); return false; }
    var s=actx.createBufferSource(); s.buffer=buffers[key]; s.connect(master); s.start(0);
  ...
  function pickVoice(){ if(!("speechSynthesis" in window))return; var vs=window.speechSynthesis.getVoices();
    nlVoice=vs.filter(function(v){return /^nl/i.test(v.lang);})[0]||null; }
  if("speechSynthesis" in window){ pickVoice(); window.speechSynthesis.onvoiceschanged=pickVoice; }
  function unlockSpeech(){ if(spUnlocked||!("speechSynthesis" in window))return;
    try{ var u=new SpeechSynthesisUtterance(" "); u.volume=0; u.lang="nl-NL"; window.speechSynthesis.speak(u); spUnlocked=true; }catch(e){} }
  function speak(t){ if(!("speechSynthesis" in window))return;
    try{ var s=window.speechSynthesis; s.resume(); if(s.speaking||s.pending)s.cancel();
      var u=new SpeechSynthesisUtterance(t); u.lang="nl-NL"; if(nlVoice)u.voice=nlVoice; u.rate=0.95; u.pitch=1.1; s.speak(u); }catch(e){} }
  ...
  function burst(x,y,n){ if(reduce)n=Math.min(n,8);
    for(var i=0;i<n;i++){var a=Math.random()*6.283,sp=3+Math.random()*7;
  ...
  function flashText(txt,x,y,color){var el=document.createElement("div");el.className="flash";el.textContent=txt;
    el.style.left=x+"px";el.style.top=y+"px";if(color)el.style.color=color;document.body.appendChild(el);
    setTimeout(function(){el.remove();},1300);}
  function centerOf(node){var r=node.getBoundingClientRect();return {x:r.left+r.width/2,y:r.top+r.height/2,top:r.top};}

  /* ---- state ---- */
  var mode="free", view="tiles";  // mode: free|quiz ; view: tiles|scene
  var current=null, locked=false, stars=0, qTimer=null;
  var CHOICES=3;
  ...
  animals.forEach(function(a,i){
    ...
    btn.className="animal"; btn.type="button"; btn.style.background=colors[i%colors.length];
    ...
    function play(){ initAudio();resume();unlockSpeech();
    ...
    btn.addEventListener("keydown",function(e){if(e.key==="Enter"||e.key===" "){e.preventDefault();play();}});
    tilesFree.appendChild(btn);
  });

  /* ---- scene handlers ---- */
  function sceneNode(key){ return sceneWrap.querySelector('.animal-node[data-key="'+key+'"]'); }
  ...
  function sceneTap(node){
    ...
    if(mode==="free"){
      ...
      if(locked) return;
      if(key===current.key){
        locked=true; node.classList.add("a-bounce","a-glow");
        bigBurst(); flashText("Goed zo! 🎉",innerWidth/2,innerHeight*0.30,"#fff");
        stars++; renderStars(); playSample(key,function(){speak(byKey[key].name);});
        qTimer=setTimeout(function(){ if(mode==="quiz"&&view==="scene"){ clearSceneFx(); newQuestion(); } },1900);
      } else {
        node.classList.remove("a-shake");void node.getBBox();node.classList.add("a-shake");
        playQuestion();
      }
    }
  }
  sceneWrap.addEventListener("pointerdown",function(e){
    var n=e.target.closest(".animal-node"); if(n) sceneTap(n);
  });
  ...
  function renderChoiceTiles(){
    var pool=shuffle(animals.slice()); var opts=[current];
    for(var i=0;i<pool.length&&opts.length<CHOICES;i++){ if(pool[i].key!==current.key) opts.push(pool[i]); }
    ...
    opts.forEach(function(a){
      ...
      btn.style.background=colors[idx[a.key]%colors.length]; btn.setAttribute("aria-label",a.name);
      btn.innerHTML='<span class="emoji" aria-hidden="true">'+a.emoji+'</span><span class="name">'+a.name+'</span>';
  ...
  function chooseTile(a,btn){
    ...
    if(a.key===current.key){
      ...
      var em=btn.querySelector(".emoji");em.classList.remove("pop");void em.offsetWidth;em.classList.add("pop");
      bigBurst(); flashText("Goed zo! 🎉",innerWidth/2,innerHeight*0.30,"#fff");
      stars++; renderStars(); playSample(current.key,function(){speak(current.name);});
      qTimer=setTimeout(function(){ if(mode==="quiz"&&view==="tiles") newQuestion(); },1900);
    } else {
      btn.classList.remove("shake");void btn.offsetWidth;btn.classList.add("shake"); playQuestion();
    }
  }
  function newQuestion(){
    locked=false;
    current=animals[(Math.random()*animals.length)|0];
    if(view==="tiles") renderChoiceTiles();
    clearTimeout(qTimer);
    setTimeout(playQuestion,430);
  }
  replayBtn.addEventListener("pointerdown",function(){ initAudio();resume();unlockSpeech(); playQuestion(); });

  /* ---- background sparkle (free tiles only, empty area) ---- */
  ...
  var tabTiles=document.getElementById("tabTiles"),tabScene=document.getElementById("tabScene");
  function render(){
    ...
    subtitle.textContent =
      mode==="free" ? (view==="scene"?"Tik op een dier in de boerderij!":"Tik op een dier!")
                    : (view==="scene"?"Zoek het dier dat je hoort!":"Welk dier hoor je?");
    if(mode==="quiz") newQuestion();
  }
  function setMode(m){ initAudio();resume();unlockSpeech(); mode=m; render(); }
  function setView(v){ initAudio();resume();unlockSpeech(); view=v; render(); }
  tabPlay.addEventListener("pointerdown",function(){setMode("free");});
  tabQuiz.addEventListener("pointerdown",function(){setMode("quiz");});
  tabTiles.addEventListener("pointerdown",function(){setView("tiles");});
  tabScene.addEventListener("pointerdown",function(){setView("scene");});
...
SCENE = open("scene.svg").read()
out = HTML.replace("__SCENE__", SCENE).replace("__SOUNDS__", SOUNDS_JS)
open("dieren.html","w").write(out)
print("Wrote dieren.html:", os.path.getsize("dieren.html"), "bytes")
